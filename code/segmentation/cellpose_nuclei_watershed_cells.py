...
files += glob("data/spacetx/plp_cumulative/rep*/*/*/nuclei-fov*.tiff")

for file in tqdm(files):
	img = io.imread(file)
	masks, flows, styles = model.eval(img, diameter=None)
	outfile = file.replace("data/spacetx", "analysis/segmentation").replace("nuclei-fov_00","fov_").replace("nuclei-fov_0","fov_").replace("-c0-r0-z0.tiff", "")
